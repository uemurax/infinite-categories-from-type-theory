...
** $\infty$-Categories
There are many definitions of $\infty$-categories[fn:nlab].
...
[fn:nlab] [[https://ncatlab.org/nlab/show/(infinity,1)-category]]
** Quasicategories
...
**** Quasicategory
     ...
     :END:
A /quasicategory/ \cite{joyal2002quasi-categories}
is a simplicial set $K$
...
**** Function
     ...
     :BEAMER_ENV: example
     :END:
Suppose $P$, $Q$ and $R$ are functional,
and $f : (a : A) (b : B) (c : C) \to
P(a, b) \times R(b, c) \to Q(a, c)$.
Then $\lambda (p, q, r) \to f(\langle p, r \rangle) = q$
is functional.
#+ATTR_LATEX: :mode math :environment xymatrix
| A |                   | Q \ar[ll] \ar[rr] |                   | C |
|   | P \ar[ul] \ar[dr] | \Uparrow f        | R \ar[dl] \ar[ur] |   |
|   |                   | B                 |                   |   |
** Strategy
   :PROPERTIES:
   :BEAMER_ENV: againframe
...
** Formalization of higher relations
   :PROPERTIES:
   :BEAMER_OPT: allowframebreaks,label=
   :END:
...
*** Clean
[[elisp:(dolist%20(f%20(remove%20(buffer-file-name)%20(directory-files%20"."%20t%20"^slide\\.")))%20(delete-file%20f))][elisp:(dolist (f (remove (buffer-file-name) (directory-files "." t "^slide\\."))) (delete-file f))]]
** Fontawesome
...
#+END_SRC
** Color theme
We use our original Beamer color theme,
[[https://github.com/uemurax/japanese-theme][Traditional Colors of Japan]].
To install it, execute the following commands:
#+BEGIN_SRC sh
  git clone https://github.com/uemurax/japanese-theme
  cd japanese-theme
  make install
#+END_SRC
** Local variables
# Local Variables:
# org-latex-pdf-process: ("bash -c \"[ -e references.bib ] || wget -O references.bib http://www.citeulike.org/bibtex/user/uemurataichi\"" "lualatex -draftmode -interaction nonstopmode %b" "bibtex %b" "lualatex -draftmode -interaction nonstopmode %b" "lualatex %b")
